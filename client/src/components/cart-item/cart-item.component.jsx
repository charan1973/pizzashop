...
import { Box, Divider, Flex, IconButton, ListItem, Text, UnorderedList } from "@chakra-ui/react";
import { useContext } from "react";
import { deleteFromCartAction, increaseDecreaseQuantityAction } from "../../context/item/item.actions";
        ...
        <Box
          ...
          >
            ...
            <Flex p="10px" justifyContent="space-between">
              ...
              <Box>
                <IconButton icon={<AddIcon />} onClick={() => itemDispatch(increaseDecreaseQuantityAction(item.id, "increase"))} />
                <Text as="span" m="5px">
                  {item.quantity}&#9932;{eachItemPrice(item)}
                </Text>
                <IconButton icon={<MinusIcon />} onClick={() => itemDispatch(increaseDecreaseQuantityAction(item.id, "decrease"))}/>
              </Box>
            </Flex>
          </Box>
     );
