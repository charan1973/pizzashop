...
exports.createOrder = async (req, res) => {
  ...
  try{
    ...
    const getOrder = await Order.findById(savedOrder._id).populate("orderContent.itemId orderContent.addOn._id")
    ...
    a + (item.itemId.size[item.itemSize] + item.addOn.reduce((b, addon) => b + addon._id.addOnPrice, 0)) * item.quantity
    ,0)
    return res.json({message: checkBill === bill ? "Order placed successfully" : "Numbers doesn't add up"})
  }catch(err){
